...
export function IncomeExpenseLineChart({ points }: { points: MonthlyTrendPoint[] }) {
  ...
  return (
    <Card>
      <p className="text-xs uppercase tracking-[0.24em] text-slate-400">{t("dashboard.chartTrendsEyebrow")}</p>
      <h2 className="mt-3 text-xl font-semibold tracking-tight text-slate-950">{t("dashboard.chartTrendsTitle")}</h2>
      <div className="mt-6 h-[320px]">
        {points.length === 0 ? (
          <div className="grid h-full place-items-center rounded-3xl bg-slate-50 px-6 text-center text-sm text-slate-500">
            {t("dashboard.chartTrendsEmpty")}
          </div>
        ) : (
          <Line
            data={{
              ...
              datasets: [
                {
                  label: t("dashboard.chartIncomeLabel"),
                ...
                {
                  label: t("dashboard.chartExpensesLabel"),
