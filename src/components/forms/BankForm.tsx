...
export function BankForm({ userId }: { userId: string }) {
  ...
  function onSubmit(values: BankInput) {
    ...
    startTransition(async () => {
      ...
      setMessage("Bank account saved locally.");
      reset({
        ...values,
        nickname: "",
        last4: "",
  ...
  return (
    <Card>
      <h2 className="text-xl font-semibold tracking-tight text-slate-950">Add a bank account</h2>
      <form className="mt-6 grid gap-4" onSubmit={handleSubmit(onSubmit)}>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <FormField label="Nickname" error={errors.nickname?.message}>
            <Input placeholder="Salary wallet" {...register("nickname")} />
          </FormField>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <FormField label="Last 4 digits" error={errors.last4?.message}>
            <Input maxLength={4} placeholder="1234" {...register("last4")} />
          </FormField>
